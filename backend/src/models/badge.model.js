...
async function create({ uid, ownerUserId = null, type = "permanent", expiresAt = null, isActive = true }) {
  // règles simples
  ...
  const { rows } = await pool.query(
    `INSERT INTO badges (uid, owner_user_id, badge_type, expires_at, is_active)
     VALUES ($1,$2,$3,$4,$5)
     RETURNING *`,
    [uid, ownerUserId, type, expiresAt, isActive]
  );
  return rows[0];
}
...
module.exports = { list, create, updateOwner, setActive };
